...
#     # Inside generate_detailed_answer function
...
#         Make sure the answer flows like a mini textbook explanation. Don't cluster everything together. Keep spacing and clarity.
...
#         print(f"\nQ: {question}\nA: {answer[:100]}...\n")  # Print just the first 100 chars of answer
#         return answer
...
# # Function to check if answers have already been generated for a question
# def is_already_answered(question, answers_json):
#     """Check if a question already has an answer in the json file"""
#     try:
...
#     """Update answers json by only generating new answers for questions not already answered"""
#     print(f"Loading questions from {questions_json}")
...
#     # Create or load existing answers data
#     try:
#         with open(answers_json, 'r', encoding='utf-8') as f:
...
#     # Create a dictionary of existing question-answer pairs for quick lookup
#     existing_qa = {pair["question"].strip(): pair["answer"] for pair in qa_pairs}
    
#     # Count of new answers generated
#     new_answers = 0
    
#     # Generate answers for questions not already answered
#     for i, question in enumerate(questions):
...
#             new_answers += 1
#         else:
#             print(f"Question {i+1}/{len(questions)} already has an answer")
...
#     print(f"Generated {new_answers} new answers. Total: {len(qa_pairs)} answers saved to {answers_json}")
#     return True
